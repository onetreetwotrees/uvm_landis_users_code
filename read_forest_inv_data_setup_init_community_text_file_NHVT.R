...
share_dir01 <- "C:\\Users\\janer\\Dropbox\\LANDIS_Data_Jane\\"
setwd(share_dir01)

## Read pre-summarized inventory data for plots in northeast kingdom study area
lstoposub <- lstoposub <- read.csv(paste(share_dir01,"csvs\\lstopospout_ALL_3.csv",sep=""),header=T) ##
plotage0 <- read.csv(paste(share_dir01,"csvs\\VTNH_AGE.csv",sep=""),header=T)
plotcoords <- read.csv(paste(share_dir01,"csvs\\VTNH_Coords.csv",sep=""),header=T)
