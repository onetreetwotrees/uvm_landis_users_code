...
pptnorm <- zpptsort %>% dplyr::group_by(ecoregion,month) %>% dplyr::filter(year %in% 1981:2010) %>% 
  dplyr::summarize_at(vars(mean),funs(sd(.,na.rm=T))) %>% select(eco=ecoregion,mo=month,pptsd=mean) %>% as.data.frame()
...
#write.csv(pptnorm,paste(rootdir,"Terraclimate_ppt_sd_normal_1981-2010_",studyarea,".csv",sep=""),row.names=F)
...
zsradsortsub <- zsradsort %>% dplyr::select(Ecoregion=ecoregion,year=year,mo=month,par=mean) %>% mutate(par=2.1*par)
zsradsortsub <- zsradsortsub %>% inner_join(modoy) %>% dplyr::select(Ecoregion,year,doy,par)
...
pnetclim$par <- round(zsradsortsub$par,digits=4)
