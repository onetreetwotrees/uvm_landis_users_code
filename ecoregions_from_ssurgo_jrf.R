...
# Extract cut points from the soils data for your study area. These will be used to bin/subdivide your soils classes later.
# Current quantiles divide slope and depth observed in your soils data into 1/3s, but you can adjust to different bins.
...
# Apply cut points. Cutpoints are the quantiles you defined about in Extract cut points section.
D2$slopebin <- as.numeric(cut2(D2$slopegradw,unique(slopecuts)))
D2$depthbin <- as.numeric(cut2(D2$maxdepth,unique(depthcuts)))

# Identify unique combinations. These will become your soils classes. This may be preliminary if you want to substitute elevation as a divider.
